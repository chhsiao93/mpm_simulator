...
def action_fn(observation):
    ...
    torque = np.clip(0.1*(np.random.normal(-dist_vec[0], 0.2)), -0.03, 0.03)
    # if np.abs(observation['wheel_omega']) < 0.5:
    #     torque = torque * 5
    return torque
...
for frame in range(400):
    ...
    gui.circle(env.target/scale, radius=3, color=0xde1a24) # target position
    gui.circles(env.observation['obs_xy']/scale, color=0x000000, radius=2.5) # observation points
